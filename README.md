...
Tool arguments are serialized once before guard evaluation; the guard and owner-approval port see the same canonical JSON that is parsed for execution. Non-JSON values, non-string top-level keys, and arguments over 4 KiB are held. The Jev adapter also sends those arguments and the user request to the configured model service, so review privacy and redact secrets before using it with real data. This is an in-process binding, **not** a cryptographic attestation of an external owner or a safe tool implementation. The integrating application must authenticate the owner, display the exact tool name and arguments, secure the approval channel, and ensure the tool dispatcher cannot be reached by the planner through another path. Never wire a model or planner into `OwnerApproval`. A declined, timed-out, or interrupted approval does not dispatch. The current `Motion.motion_class` is not a calibrated motion safety assessment. Do not enable effectful tools or real motion without trusted adapters and integration tests. With no Reachy Mini available, all output-order tests use spies only. Run the [offline ordering example](examples/guarded_turn.py) with `uv run python examples/guarded_turn.py`; it uses fake verdicts and no Jev call or robot.
