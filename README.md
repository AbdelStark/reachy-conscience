...
This repository contains the tested guard core, local ledger, and an independently owned, non-streaming conversation-path foundation. It does **not** wrap or protect the official Conversation App. Bounded audio ingress, optional local ASR, a proposal-only local LLM adapter, and an offline TTS fallback exist, but they are not yet wired into a deployable robot app with a trusted owner UI and validated output lifecycle. The SDK audio and motion adapters need on-robot validation. It is **not** a physical safety system, prompt-injection cure, or substitute for motor limits, torque limits, firmware emergency stop, or owner supervision. No block-rate or latency claim has been measured.
...
The host must explicitly arm playback after capture stops and wire the same owned audio output to the guarded pipeline. Do not run another microphone or speech output path in parallel. A transcript equal to the local hard-stop command must reach `run_turn()` without waiting for a planner or model. An ASR error or timeout must not be treated as approval or a reason to run a proposal. This repository does not yet ship a continuously running voice loop or claim hands-free stop reliability.
